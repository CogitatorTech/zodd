    ...
    try minish.check(
        ...
        struct {
            fn prop(lists: TwoLists) !void {
                const allocator = testing.allocator;
                var rel1a = try zodd.Relation(u32).fromSlice(allocator, lists[0]);
                var rel2a = try zodd.Relation(u32).fromSlice(allocator, lists[1]);
                var merged_ab = try rel1a.merge(&rel2a);
                defer merged_ab.deinit();

                var rel1b = try zodd.Relation(u32).fromSlice(allocator, lists[0]);
                var rel2b = try zodd.Relation(u32).fromSlice(allocator, lists[1]);
    ...
    try minish.check(
        ...
        struct {
            fn prop(edges: []const Edge) !void {
                const allocator = testing.allocator;
                var edges_rel = try zodd.Relation(Edge).fromSlice(allocator, edges);
...
test "property: relation merge is idempotent" {
    try minish.check(
        ...
        struct {
            fn prop(data: []const u32) !void {
                const allocator = testing.allocator;
                var rel1 = try zodd.Relation(u32).fromSlice(allocator, data);
                defer rel1.deinit();

                var rel2 = try zodd.Relation(u32).fromSlice(allocator, data);
                var merged = try rel2.merge(&rel1);
                defer merged.deinit();

                var expected = try zodd.Relation(u32).fromSlice(allocator, data);
    ...
    try minish.check(
        ...
        struct {
            fn prop(input: Pair) !void {
                const allocator = testing.allocator;
                var rel = try zodd.Relation(u32).fromSlice(allocator, input[0]);
                defer rel.deinit();
    ...
    try minish.check(
        ...
        struct {
            fn prop(lists: ThreeLists) !void {
                const allocator = testing.allocator;
                var a1 = try zodd.Relation(u32).fromSlice(allocator, lists[0]);
                var b1 = try zodd.Relation(u32).fromSlice(allocator, lists[1]);
                var c1 = try zodd.Relation(u32).fromSlice(allocator, lists[2]);

                var ab = try a1.merge(&b1);
                var ab_c = try ab.merge(&c1);
                defer ab_c.deinit();

                var a2 = try zodd.Relation(u32).fromSlice(allocator, lists[0]);
                var b2 = try zodd.Relation(u32).fromSlice(allocator, lists[1]);
                var c2 = try zodd.Relation(u32).fromSlice(allocator, lists[2]);
...
test "property: joinHelper matches naive join" {
    ...
    try minish.check(
        ...
        struct {
            fn prop(p: Pair) !void {
                const allocator = testing.allocator;
                var rel1 = try zodd.Relation(Tuple).fromSlice(allocator, p[0]);
                defer rel1.deinit();

                var rel2 = try zodd.Relation(Tuple).fromSlice(allocator, p[1]);
                defer rel2.deinit();
                ...
                var expected = try zodd.Relation(Result).fromSlice(allocator, expected_list.items);
                defer expected.deinit();
                ...
                var got = try zodd.Relation(Result).fromSlice(allocator, got_list.items);
                defer got.deinit();
    ...
    try minish.check(
        ...
        struct {
            fn prop(p: Pair) !void {
                ...
                var got = try zodd.Relation(Tuple).fromSlice(allocator, output.recent.elements);
                defer got.deinit();
    ...
    try minish.check(
        ...
        struct {
            fn prop(p: Pair) !void {
                ...
                var rel = try zodd.Relation(KV).fromSlice(allocator, p[1]);
                defer rel.deinit();

                var output = zodd.Variable(KV).init(allocator);
                defer output.deinit();

                try source.insertSlice(p[0]);
                _ = try source.changed();

                var ext = zodd.ExtendWith(Tuple, u32, u32).init(allocator, &rel, struct {
                ...
                var expected = try zodd.Relation(KV).fromSlice(allocator, expected_list.items);
                defer expected.deinit();

                var got = try zodd.Relation(KV).fromSlice(allocator, output.recent.elements);
                defer got.deinit();
    ...
    try minish.check(
        ...
        struct {
            fn prop(data: List) !void {
                const allocator = testing.allocator;
                const Index = zodd.SecondaryIndex(Tuple, u32, struct {
                ...
                var rel = try zodd.Relation(Tuple).fromSlice(allocator, data);
                defer rel.deinit();
    ...
    try minish.check(
        ...
        struct {
            fn prop(data: List) !void {
                ...
                var result = try zodd.aggregate(Tuple, u32, u32, allocator, &rel, key_func.key, 0, folder.fold);
                defer result.deinit();
                ...
                var expected = try zodd.Relation(struct { u32, u32 }).fromSlice(allocator, expected_list.items);
                defer expected.deinit();
    ...
    try minish.check(
        ...
        struct {
            fn prop(data: List) !void {
                ...
                var original = try zodd.Relation(Tuple).fromSlice(allocator, data);
                ...
                var loaded = try zodd.Relation(Tuple).load(allocator, &reader);
                defer loaded.deinit();
